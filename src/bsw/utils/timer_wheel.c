...
int mod_timer(void *ptimer, unsigned long expires) {
    ...
    if (NULL == timer->function) {
        LOG_ERROR("timer's timeout function is null");
        return -1;
    }
...
static void *deal_function_timeout(void *base) {
    ...
    for (;;) {
        ...
        while (ba->current_index <= (tv.tv_sec * 1000 + tv.tv_usec / 1000)) {
            ...
            // 指针指向0槽时，级联轮需要更新任务列表
            if (!index && (!cascade(ba, &ba->tv2, INDEX(0))) && (!cascade(ba, &ba->tv3, INDEX(1))) &&
...
void *ti_timerwheel_create(void) {
    ...
    mmeset(base, 0, sizeof(struct tvec_base));
