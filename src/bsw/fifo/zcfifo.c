 ...
 * &zcfifo_t with kfree().
 */
zcfifo_t *zcfifo_init(unsigned char *buffer, unsigned int size, zcfifo_lock_t *lock)
{
	zcfifo_t *fifo;
 ...
 */
zcfifo_t *zcfifo_alloc(unsigned int size, zcfifo_lock_t *lock)
{
	...
	ret = zcfifo_init(buffer, size, lock);
...
/**
 * __zcfifo_put - puts some data into the FIFO, no locking version
 ...
 */
unsigned int __zcfifo_put(zcfifo_t *fifo,
...
/**
 * __zcfifo_get - gets some data from the FIFO, no locking version
 * @fifo: the fifo to be used.
 ...
 * writer, you don't need extra locking to use these functions.
 */
unsigned int __zcfifo_get(zcfifo_t *fifo,
