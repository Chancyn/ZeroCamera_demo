...
extern int rtsp_transport_tcp_create(socket_t socket, const struct sockaddr *addr, socklen_t addrlen,
                                     struct aio_rtsp_handler_t *handler, void *param);

static void rtsp_server_onaccept(void *param, int code, socket_t socket, const struct sockaddr *addr,
                                 socklen_t addrlen) {
...
void *rtsp_server_listen(const char *ip, int port, struct aio_rtsp_handler_t *handler, void *param) {
    ...
    // create server socket zhoucc resue 1
