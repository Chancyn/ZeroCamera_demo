...
CTsDemuxer::CTsDemuxer() : Thread("tsmuxer"), m_Idr(false), m_status(ts_status_init), m_ts(nullptr) {
...
void *CTsDemuxer::tsAlloc(void *ptr, uint64_t bytes) {
    CTsDemuxer *self = reinterpret_cast<CTsDemuxer *>(ptr);
    return self->_tsAlloc(bytes);
}

void *CTsDemuxer::_tsAlloc(uint64_t bytes) {
    ZC_ASSERT(bytes == ZC_N_TS_PACKET);
    return m_pkgbuf;
}

int CTsDemuxer::tsWrite(void *ptr, const void *packet, size_t bytes) {
    CTsDemuxer *self = reinterpret_cast<CTsDemuxer *>(ptr);
    return self->_tsWrite(packet, bytes);
}

int CTsDemuxer::_tsWrite(const void *packet, size_t bytes) {
    ZC_ASSERT(bytes == ZC_N_TS_PACKET);
...
void CTsDemuxer::tsFree(void *ptr, void *packet) {
    CTsDemuxer *self = reinterpret_cast<CTsDemuxer *>(ptr);
    return self->_tsFree(packet);
}

void CTsDemuxer::_tsFree(void *packet) {
    return;
}

bool CTsDemuxer::Create(const zc_tsmuxer_info_t &info) {
    if (m_ts) {
        return false;
    }
...
int CTsDemuxer::_checkPts(zc_frame_t *frame) {
...
int CTsDemuxer::_tsAddStream(zc_frame_t *frame) {
    int avtype = 0;
...
int CTsDemuxer::_packetTs(zc_frame_t *frame) {
    int ret = 0;
    ret = _checkPts(frame);
    if (ret < 0) {
        return 0;
...
int CTsDemuxer::_getDate2PacketTs(CShmStreamR *stream) {
    int ret = 0;
