...
const char *CModReqCli::GetUrlbymodid(ZC_U8 modid) {
    return get_url_bymodid(modid);
}

CModReqCli::CModReqCli(ZC_U8 modid, ZC_U32 version) : m_modid(modid), m_seqno(0), m_version(version) {
    m_pid = getpid();
...
CModReqCli::~CModReqCli() {}

bool CModReqCli::BuildReqMsgHdr(zc_msg_t *pmsg, ZC_U8 modidto, ZC_U16 id, ZC_U16 sid, ZC_U8 chn, ZC_U32 size) {
    if (!pmsg) {
...
bool CModReqCli::MsgSendTo(zc_msg_t *pmsg, const char *urlto, zc_msg_t *prmsg, size_t *buflen) {
    pmsg->ts = zc_system_time();
    pmsg->seq = m_seqno++;
...
bool CModReqCli::MsgSendTo(zc_msg_t *pmsg, zc_msg_t *prmsg, size_t *buflen) {
    pmsg->ts = zc_system_time();
    pmsg->seq = m_seqno++;
    CMsgCommReqClient cli;
...
// send get streaminfo
int CModReqCli::sendSMgrGetInfo(unsigned int type, unsigned int chn, zc_stream_info_t *info) {
...
// send set streaminfo
int CModReqCli::sendSMgrSetInfo(unsigned int type, unsigned int chn, zc_stream_info_t *info) {
    if (info == nullptr)
        return -1;
