...
#define ZC_RTSP_TEST_FRAME_MAXSIZE ZC_STREAM_MAXFRAME_SIZE
#define ZC_RTSP_TEST_MAIN_SIZE ZC_STREAM_MAIN_VIDEO_SIZE
#define ZC_RTSP_TEST_SUB_SIZE ZC_STREAM_SUB_VIDEO_SIZE
#define ZC_RTSP_TEST_SHM_PATH ZC_STREAM_VIDEO_SHM_PATH
#define ZC_RTSP_VIDEO_MAGIC ZC_FRAME_VIDEO_MAGIC  // "ZCVE" ZC_FRAME_VIDEO_MAGIC

#define ZC_PUT_APPEND 1
static void* g_zctestfifow[HI_VENC_MAX_CHN_NUM] = {0};
...
static hi_void sample_comm_venc_set_h265_cvbr_bit_rate(hi_venc_chn_attr *venc_chn_attr, hi_venc_h264_cvbr *h265_cvbr,
...
static hi_s32 zc_sample_comm_streamw_put(hi_s32 index, hi_venc_stream *stream, hi_venc_stream_buf_info *stream_buf_info, hi_payload_type *payload_type)
{
    ...
    unsigned char buftmp[ZC_RTSP_TEST_FRAME_MAXSIZE];
    unsigned int framelen = 0;
    #if 1  // TODO(zhoucc) now just copy for test
    hi_u32 i;
    for (i = 0; i < stream->pack_cnt; i++) {
        if (framelen + stream->pack[i].len - stream->pack[i].offset <= sizeof(buftmp)) {
            memcpy(buftmp + framelen, stream->pack[i].addr + stream->pack[i].offset, stream->pack[i].len - stream->pack[i].offset);
                framelen  += stream->pack[i].len - stream->pack[i].offset;
        } else {
            ZC_ASSERT(0);
        }
    ...
    #endif
    if (stream->h264_info.ref_type == HI_VENC_BASE_IDR_SLICE) {
        LOG_WARN("put IDR len[%d]", framelen);
    }

    zc_shmfifow_put(g_zctestfifow[index], buftmp, framelen);
    ...
    hi_venc_stream_buf_info *stream_buf_info, hi_payload_type *payload_type)
{
    ...
    #if ZC_RTSP_TEST
    //send to rtsp
    zc_sample_comm_streamw_put(index, &stream, stream_buf_info, payload_type);
    #else
