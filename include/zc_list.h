...
// Modify 1.#define READ_ONCE/WRITE_ONCE from linux/compiler.h
// Modify 2.#define offsetof/container_of
// Modify 3.#define list_head/hlist_head/hlist_node
// Modify 4.Modify new->_new, for compatible with C++ compiler
 ...
 */
#define LIST_POISON1  NULL //((void*)0)//((void *) 0x00100100)
#define LIST_POISON2  NULL//((void*)0)//((void *) 0x00200200)

/*
 * These are define WRITE_ONCE/READ_ONCE
 * copy from linux/compiler.h
 */
 #ifndef WRITE_ONCE
...
//#define WRITE_ONCE(x, val) (*((volatile typeof(val) *)(&(var))) = (val))
#endif
 ...
 */
extern bool __list_valid_slowpath __list_add_valid_or_report(struct list_head *_new,
							     struct list_head *prev,
							     struct list_head *next);
 ...
 */
static __always_inline bool __list_add_valid(struct list_head *_new,
					     struct list_head *prev,
					     struct list_head *next)
{
	bool ret = true;
	...
	if (!IS_ENABLED(CONFIG_DEBUG_LIST)) {
		 ...
		 */
		if (likely(next->prev == prev && prev->next == next && _new != prev && _new != next))
			return true;
		ret = false;
	}

	ret &= __list_add_valid_or_report(_new, prev, next);
	return ret;
}
...
/**
 * hlist_add_behind - add a _new entry after the one specified
 * @n: _new entry to be added
